public class Mountain{
    ...
    static int peakMountain(int arr[]) {
        ...
        // start and end are always trying to find max elemtn in the above 2 checks
        // hence, when they are pointing to just one element , that is max one bacasue that is what the checs say
        // more elaboration : at every point for start and end, they have thebest possible ans till that time
        // and if we are saying that only item is remaining , hence cuz of above line that is bes possible ans  
         return start; // or returnend as botha are == pointing to same elemnt
    }
}